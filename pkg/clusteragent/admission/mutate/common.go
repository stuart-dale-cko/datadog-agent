...
func injectEnv(pod *corev1.Pod, env corev1.EnvVar) error {
	if pod == nil {
		return errors.New("cannot inject env var into nil pod")
	}
	for i, ctr := range pod.Spec.Containers {
		if contains(ctr.Env, env.Name) {
			log.Debugf("Ignoring container '%s' in pod '%s': env var '%s' already exist", ctr.Name, pod.Name, env.Name)
			continue
		}
