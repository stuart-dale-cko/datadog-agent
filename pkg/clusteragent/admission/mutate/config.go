...
func injectConfig(pod *corev1.Pod) error {
	...
	if shouldInjectConf(pod) {
		log.Debugf("Injecting env var DD_AGENT_HOST into pod %s/%s", pod.GetNamespace(), pod.GetName())
		return injectEnv(pod, agentHostEnvVar)
	}
	return nil
}
...
func shouldInjectConf(pod *corev1.Pod) bool {
	...
	if val, found := pod.GetLabels()[admission.EnabledLabelKey]; found {
		...
		case "false":
			return false
		default:
			log.Warnf("Invalid label value '%s=%s' on pod %s/%s should be either 'true' or 'false', ignoring it", admission.EnabledLabelKey, val, pod.GetNamespace(), pod.GetName())
			return false
		}
	}
